> この版は「1町丁目 = 1巡回ルート」に固定し、Web画面の担当人数指定を廃止しています。チームタブは同一ルートの進捗共有用として残しています。

# Posting Navigator v1.1.2 — ブロック完結・櫛型・蛇行巡回版



### v1.1.2 の優先ルール

- 幹道路に並ぶ行き止まり（櫛の歯）は、その幹道路を通ったタイミングで順番に往復して完了します。
- 格子状・ブロック状の住宅地は、同じ道路軸をなるべく端まで進み、隣の列へ移る蛇行型を優先します。
- 一部だけ残したブロックへ後から戻る巡回を強く減点します。
- 距離最小化は上記の「分かりやすく完結する」条件の中で行います。
- 公園内の不要園路・町丁目外道路を除外する single-route-filtered 方針は維持します。

## v1.1.2 巡回順の改善
- クラスタごとの個別Euler化を廃止し、町丁目全体でOpen Chinese Postmanを1回だけ計算。
- 同距離のEuler候補を複数探索し、即時折返し・180度Uターン・内側道路の取り残しを最小化。
- 道路途中での不自然な折返しを避け、折返しは原則として行き止まり等の必要箇所に限定。
- START付近を残して遠方へ飛び、後で戻る順序を減点。

## v1.1.2 巡回戦略

- 1町丁目＝1ルート。複数担当者への自動分割は行いません。
- START周辺の道路を局所クラスタとして先に処理し、近い塊から外側へ広がります。
- 各クラスタは入口→外側出口の open Chinese Postman trail で処理し、近所へ戻る往復を抑えます。
- クラスタ間の移動も実際の道路グラフ上の最短経路のみを使い、道路外の直線は作りません。
- 公園内の配布価値がない園路と町丁目外道路は引き続き巡回対象から除外します。
- 実道路で接続していない孤立成分は偽ルートで接続せず、診断値 `skipped_disconnected_length_m` に計上します。

## v1.0.9aa 境界外ナビ修正
- 境界道路検出用8mバッファと、実際に歩行可能な1.5m許容帯を分離。
- 境界外側は境界と平行な道路だけ例外採用し、外向きに伸びる枝道を除外。
- ③/⑩のような町丁目外へ曲がる案内の原因となる外側スタブを巡回グラフへ入れない。
