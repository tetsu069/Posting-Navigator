# Posting Navigator v1.0.18

## v1.0.18 修正

- 配布道路と移動道路を別々にグラフ化せず、同一OSM道路ネットワークを一度だけnode化。
- その共通ネットワークから `posting_target` edgeだけを配布対象として抽出。
- クリップ/浮動小数点差で同一交差点が0〜1.25mずれた場合、近接ノードを自動スナップ。
- スナップは独立した直線コネクタを作らず、既存道路edgeの端点だけを共通交差点へ揃える。
- 接続不能時は成分間の最短ギャップ距離をエラーに含め、診断しやすくした。
