...
function renderAreaBoundaries(fitSelected=false){
  ...
  state.layers.areas=L.geoJSON(state.areaGeojson,{
    style:f=>{
      ...
      return active
        ? {color:'#64748b',weight:4,opacity:.82,fillColor:'#cbd5e1',fillOpacity:.06}
        : {color:'#64748b',weight:2.5,opacity:.62,fillColor:'#94a3b8',fillOpacity:.025};
    },
...
function loadWorker(workerId){state.workerId=workerId;const feat=workerFeatures().find(f=>+f.properties.worker_id===workerId);if(!feat)return;const geom=feat.geometry||{},parts=geom.type==='MultiLineString'?(geom.coordinates||[]):[(geom.coordinates||[])];state.segments=[];state.segmentLengths=[];for(const coords of parts){for(let i=0;i<coords.length-1;i++){state.segments.push([coords[i],coords[i+1]]);state.segmentLengths.push(haversine(coords[i][1],coords[i][0],coords[i+1][1],coords[i+1][0]))}}const key=progressKey();let saved=[];try{saved=JSON.parse(localStorage.getItem(key)||'[]')}catch{}state.completed=new Set(saved);state.fieldGuideLeg=0;state.lastPosition=null;$('fieldWorkerName').textContent=feat.properties.name||`担当${workerId}`;drawFieldRoute();updateFieldProgress();nextRouteInstruction();pullProgress()}
function progressKey(){return `pn_progress_${state.projectId||'local'}_${state.workerId}`}
function segmentPhysicalKey(s){const q=p=>`${(+p[0]).toFixed(6)},${(+p[1]).toFixed(6)}`;const a=q(s[0]),b=q(s[1]);return a<b?`${a}|${b}`:`${b}|${a}`}
function drawFieldRoute(){if(!state.areaGeojson&&state.geojson){const areaOnly={type:'FeatureCollection',features:(state.geojson.features||[]).filter(f=>f.properties?.kind==='area')};if(areaOnly.features.length){state.areaGeojson=areaOnly;renderAreaBoundaries(false)}}if(state.layers.generated){state.layers.generated.remove();state.layers.generated=null}['todo','once','roundtrip','done','directions','sequence'].forEach(k=>{if(state.layers[k]){state.layers[k].remove();state.layers[k]=null}});const todo=[],once=[],roundtrip=[];const doneCounts=new Map();state.segments.forEach((s,i)=>{if(state.completed.has(i)){const k=segmentPhysicalKey(s);doneCounts.set(k,(doneCounts.get(k)||0)+1)}});state.segments.forEach((s,i)=>{const f={type:'Feature',properties:{segment:i},geometry:{type:'LineString',coordinates:s}};if(!state.completed.has(i)){todo.push(f);return}const n=doneCounts.get(segmentPhysicalKey(s))||0;(n>=2?roundtrip:once).push(f)});state.layers.todo=L.geoJSON({type:'FeatureCollection',features:todo},{style:{color:'#ef4444',weight:7,opacity:.66}}).addTo(map);state.layers.once=L.geoJSON({type:'FeatureCollection',features:once},{style:{color:'#2563eb',weight:8,opacity:.92}}).addTo(map);state.layers.roundtrip=L.geoJSON({type:'FeatureCollection',features:roundtrip},{style:{color:'#22c55e',weight:9,opacity:.96}}).addTo(map);drawNextWorkerArrow();const both=L.featureGroup([state.layers.todo,state.layers.once,state.layers.roundtrip]);const b=both.getBounds();if(b.isValid())map.fitBounds(b,{padding:[20,20]})}
