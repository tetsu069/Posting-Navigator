...
def overpass_query(poly: Polygon) -> str:
    ...
    # administrative boundary.  v1.0.21 keeps that halo as transfer-only roads.
...
def _headers() -> dict[str, str]:
    ...
    user_agent = os.getenv(
        "OVERPASS_USER_AGENT",
        "Posting-Navigator/1.0.21 (+https://tetsu069.github.io/Posting-Navigator/)",
    ).strip()
    referer = os.getenv(
        "OVERPASS_REFERER",
...
def osm_json_to_mobility_lines(data: dict, boundary: Polygon) -> list[dict]:
    ...
    v1.0.21 separates *where we distribute* from *where we are allowed to walk*.
