...
def build(args: argparse.Namespace) -> int:
    ...
    (out / "summary.json").write_text(json.dumps({**{k:v for k,v in route.items() if k not in {"geometry", "start_point", "requested_start", "assignments", "route_steps"}}, "assignments": [{k:v for k,v in a.items() if k not in {"geometry", "start_point", "end_point"}} for a in route["assignments"]]}, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({**{k:v for k,v in route.items() if k not in {"geometry", "start_point", "requested_start", "assignments", "route_steps"}}, "assignments": [{k:v for k,v in a.items() if k not in {"geometry", "start_point", "end_point"}} for a in route["assignments"]]}, ensure_ascii=False, indent=2))
    return 0
